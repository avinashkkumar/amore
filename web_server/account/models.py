...
class MyAccountManager(BaseUserManager):
	...
	def create_student(self, firstName, lastName, email, username, password=None):
		user = self.model(
			email=self.normalize_email(email),
			username=username,
			firstName = firstName,
		...
		user.set_password(password)
		user.is_student_member = True
		user.save(using=self._db)
		return user
	
	def create_teacher(self, firstName, lastName, username, email, password=None):
		user = self.model(
			email=self.normalize_email(email),
			username= username,
			firstName = firstName,
